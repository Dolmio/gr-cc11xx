from operator import xor
from array import array

#https://github.com/andrepuschmann/gr-cc11xx/blob/fa1189fe8a1f600129b00c053ce5abd4da80e6be/lib/cc11xx_utility.h
pn9_table = [
    
]

#Code taken from CRC app note: http://www.ti.com/lit/an/swra111d/swra111d.pdf
def calc_crc_step(prev_crc, byte):
    crc_poly = 0x8005
    for i in range(8):
        if((prev_crc & 0x8000) >> 8) ^ (byte & 0x80):
            prev_crc = (prev_crc << 1) ^ crc_poly
        else:
            prev_crc = (prev_crc << 1)
        byte <<= 1
    return prev_crc


def crc16(buffer):
    crc = reduce(calc_crc_step, buffer, 0xFFFF)
    return [(crc >> 8) & 0xff, crc & 0xff]

#this a custom crc-calculation implemented in the radio firmware by SpaceFactory
def crc16_aalto(buffer):
    crc16_lookup = [
        0x0000, 0x1081, 0x2102, 0x3183,
        0x4204, 0x5285, 0x6306, 0x7387,
        0x8408, 0x9489, 0xa50a, 0xb58b,
        0xc60c, 0xd68d, 0xe70e, 0xf78f
    ]

    crc = 0xFFFF;
    length = len(buffer)
    pos = 0
    while pos < length:
        crc = (crc >> 4) ^ crc16_lookup[(crc & 0xf) ^ (buffer[pos] & 0xf)]
        crc = (crc >> 4) ^ crc16_lookup[(crc & 0xf) ^ (buffer[pos] >> 4)]
        pos += 1
    data = crc
    crc = ((crc << 8) | (data >> 8 & 0xff)) & 0xffff
    crc_word = crc ^ 0xffff
    return [(crc_word >> 8) & 0xff, crc_word & 0xff]

#http://www.ti.com/lit/an/swra322/swra322.pdf
def whiten(data):
    return [xor(byte, pn9_table[i]) for i, byte in enumerate(data)]

def calc_max_data_length(crc):
    crc_length = 2 if crc else 0
    length_field_length = 1
    address_field_length = 1
    return len(pn9_table) - crc_length - length_field_length - address_field_length

def create(message_arr, preamble=[0x20, 0x20, 0x20, 0x20, 0x20, 0x20], syncword=[0x35, 0x2E, 0x35, 0x2E], crc=True, whitening=True, crc_f=crc16):
    data_max_length = calc_max_data_length(crc) 
    if len(message_arr) > data_max_length:
        raise ValueError("Max length for packet is: %d " % data_max_length)
    payload = [len(message_arr)] + message_arr
    with_crc = payload + crc_f(payload) if crc else payload
    with_whitening = whiten(with_crc) if whitening else with_crc
    packet = preamble + syncword + with_whitening
    return bytearray(packet)

if __name__ == "__main__":
    data = "ABCDE"
    packet = create(data)

    with open("packet.bin", "wb") as f:
        f.write(packet)
